...
class OrganizationService extends ManagerService
{
    ...
    public function addButton()
    {
        ...
        $button   =
            $fabrique->createButton("Ajouter un établissement", "fa fa-plus", "white-text text-lighten-4 light-green darken-4");
    ...
    public function findAll()
    {
        ...
        if ($records) {
            foreach ($records as $record) {
                ...
                $cellAction->setCellattribute($this->getCellAttribute("fa fa-edit", "Modifier", "organization_upd", "light-blue darken-3 white-text"));
                $cell->setCellAction($cellAction);
                $row->addCells($cell);
                
                $cell       = $this->getCell("del", "", "cell-action");
                $cellAction = $this->getCellAction("upd", "link");
                $cellAction->setCellattribute($this->getCellAttribute("fa fa-trash", "Supprimer", "organization_del", "red darken-3 white-text"));
                $cell->setCellAction($cellAction);
                $row->addCells($cell);
